...
.user-bubble {background-color: #D9FDD3; color: #111b21; padding: 10px 14px; border-radius: 18px; border-bottom-right-radius: 4px; max-width: 75%; margin-left: auto; margin-bottom: 10px; text-align: right; white-space: pre-wrap; word-wrap: break-word;}
pre {background: #0d0d0d!important; border: 1px solid #333!important; border-radius: 10px!important; padding: 12px!important;}
...
def generate_image_url(prompt, is_school_mode, age):
    if is_school_mode:
        if "1-2" in age or "3-4" in age: final_prompt = f"cute baby cartoon, very simple, bright colors, 3d pixar style, {prompt}, no person"
        else: final_prompt = f"kid friendly educational diagram, colorful, {prompt}, no person"
    else: final_prompt = f"realistic, cinematic, 4k, {prompt}"
    try:
        ...
        if hf_key:
            ...
            if r.status_code == 200 and len(r.content) > 1000: return r.content, "huggingface"
    except: pass
...
NORMAL_SYSTEM_PROMPT = "You are ClyxessChat AI. REPLY ONLY IN SAME LANGUAGE AS USER. Friendly. If hi/namaste -> reply namaste style. If image asked -> 'Generating image for: [prompt]'"

def get_school_system_prompt(age_group):
    base = f"You are ClyxessChat AI - School Mode. Age Group: {age_group}. SAFETY: No adult, no violence. "
    if "1-2" in age_group: return base + " Didi for 1-2 yrs. Max 8 words. Rhymes, colors, emojis. If hi/namaste -> 'Namaste baby! 😊 Laal gubbara 🎈'"
    elif "3-4" in age_group: return base + " Didi for 3-4 yrs. Max 12 words. Sounds. If hi/namaste -> 'Namaste! 😊 Chidiyaghar chalen? 🦁'"
    elif "5-6" in age_group or "6-8" in age_group: return base + " Age 5-8: Curiosity. Max 30 words. If hi/namaste -> 'Namaste! Kaise ho? 😊 Puzzle karen?'"
    elif "10-11" in age_group: return base + " Age 10-11: Maker. 50-60 words. If hi/namaste -> 'Namaste! Badhiya hu, aap kaise ho? Rocket banayen?' If code asked give full centered HTML/CSS/JS code."
    else: return base + " Age 11+: Future Tech, AI. No limit. If hi/namaste -> 'Namaste! Kaise ho? Ready ho coding ke liye? 💻' If asks programming write full clean code."
...
def fetch_duolingo_task(client, age, lang, level):
    if "1-2" in age or "3-4" in age:
        task_type = random.choice(["color_match", "animal_sound", "tap_image"])
        prompt = f"Age {age}, Lang {lang}, Level {level}, Task {task_type}. JSON only: {{\"question\":\"short question with emoji in {lang}\", \"options\":[\"A\",\"B\",\"C\"], \"answer\":\"correct option\", \"xp\":10}}"
    elif "5-6" in age or "6-8" in age:
        task_type = random.choice(["translate", "shape_puzzle", "math_small"])
        prompt = f"Age {age}, Lang {lang}, Level {level}, Task {task_type}. JSON only: {{\"question\":\"question in {lang}\", \"options\":[\"A\",\"B\",\"C\",\"D\"], \"answer\":\"correct\", \"xp\":15}}"
    else:
        task_type = random.choice(["translate", "coding_logic", "science_why", "math_big"])
        prompt = f"Age {age}, Lang {lang}, Level {level}, Task {task_type}. JSON only: {{\"question\":\"question in {lang}\", \"options\":[\"A\",\"B\",\"C\",\"D\"], \"answer\":\"correct\", \"xp\":20}}"
...
def duolingo_game_mode(client):
    ...
    if st.session_state.d_task is None:
        with st.spinner(f"{sel_lang} me naya task bana raha hu..."):
    ...
    for opt in task['options']:
        if st.button(opt, key=f"opt_{opt}_{st.session_state.d_done}_{random.randint(1,1000)}", use_container_width=True):
    ...
    with colA:
        if st.button("💡 Hint"): st.info(f"Hint: '{task['answer'][:1]}...' se start hota hai")
    with colB:
...
if prompt := st.chat_input("Apna idea type karein ya draw karein..." if "Creative" in mode else "Ask ClyxessChat AI"):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("ClyxessChat AI is responding..."):
            ...
            if completion is None: st.error("Model busy hai, 1 min baad try karo"); st.stop()
