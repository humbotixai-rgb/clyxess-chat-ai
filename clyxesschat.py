...
def _age_subject_fallback(age, subject, language, count):
    ...
    # For Hindi, use the existing age fallback, but if subject is clearly older-level
    # prefer the subject's own QUESTION_BANK over generic baby content.
    if language == "hi" and age in ("8–10 Years","10–11 Years","11+ Years") and subject in QUESTION_BANK:
        bank=QUESTION_BANK.get(subject,[])
        if bank:
...
def render_vision_lab():
    ...
    labels=list(PLAY_LANGUAGES.keys()); label=st.selectbox("Answer language",labels,index=labels.index("🇮🇳 हिंदी"),key
